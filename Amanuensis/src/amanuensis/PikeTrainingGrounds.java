...
/**class PikeTrainingGrounds
 * 
 * Building for creating pikeman
 * @author Hexahedron
 */

public class PikeTrainingGrounds extends Building{
    ...
    public PikeTrainingGrounds(Lane lane, boolean isHost, int level) {
        super(lane, isHost, level);
        canAttack = false;
        hP = HIT_P;
        mod = new ImageModificator();
        loader = new ImageLoader("res/pikeaRED.png");
        imgU = mod.resize(loader.getIMG(), 6*size*size_modifier/loader.getIMG().getWidth());
        loader = new ImageLoader("res/pikeaBLUE.png");
        imgA = mod.resize(loader.getIMG(), 6*size*size_modifier/loader.getIMG().getWidth());
    }
    public PikeTrainingGrounds(FieldModel fM, boolean isHost, int level)
    {
